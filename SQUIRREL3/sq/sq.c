...
void begin_read(const char* prompt)
{
    static char* line;
...
void new_read(const char* prompt)
{
	free(line);
	begin_read(prompt);
}
...
#define getchar rl_getchar

#else
#define begin_read(x) //nop
#define end_read() //nop
#define new_read(x) //nop
#endif
...
#ifdef READLINE
#define NEWLINE__ '\0'
#define if_not_rl(x) //nop
#else
#define NEWLINE__ '\n'
#define if_not_rl(x) x
#endif

    while (!done) 
	{
		SQInteger i = 0;
		if_not_rl(scprintf(_SC("\nsq>")));
		begin_read("\nsq>");
		for(;;) {
			int c;
			if(done)return;
