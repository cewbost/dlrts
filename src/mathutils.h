...
namespace Math
{
	constexpr unsigned int MaxUInt32 = 0xFFFFFFFF;
	constexpr int MinInt32 = 0x80000000;
	constexpr int MaxInt32 = 0x7FFFFFFF;
	constexpr float MaxFloat = 3.402823466e+38F;
	constexpr float MinPosFloat = 1.175494351e-38F;
	...
	constexpr float ZeroEpsilon = 32.0f * MinPosFloat;  // Very small epsilon for checking against 0.0f
#ifdef __GNUC__
	constexpr float NaN = __builtin_nanf("");
#else
	constexpr float NaN = *(float *)&MaxUInt32;
#endif
...
static inline float vecAngle(float x, float y)
{
    return std::atan2(y, x) * 57.29577951f;
}
